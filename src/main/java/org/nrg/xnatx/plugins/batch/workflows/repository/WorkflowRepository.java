...
public class WorkflowRepository implements PageableRepository {
    ...
    private static final String PARAM_NUM_DAYS   = "days";
    private static final String PARAM_ROW_LIMIT  = "rowLimit";
    private static final String PARAM_ROW_OFFSET = "rowOffset";
    ...
    private static final RowMapper<Workflow> WF_ROW_MAPPER = (resultSet, index) -> {
        ...
        for (int i = 1; i <= metaData.getColumnCount(); i++) {
            ...
            ColumnDataType columnDataType = COLUMN_INFO.get(columnName);
            assert columnDataType != null;
            Class<?> columnClass = columnDataType.dataType;
            if (columnClass.equals(Timestamp.class)) {
                item        = new Date(((Timestamp) item).getTime());
                columnClass = Date.class;
            }
            w.setProperty(columnDataType.columnName, item, columnClass);
        }
        return w;
    };
    ...
    private static final String QUERY_RECENT_EXPTS = "SELECT * " +
                                                     ...
                                                     "                              wrk.id, " +
                                                     "                              wrk.externalid, " +
                                                     "                              CASE WHEN wrk.pipeline_name = 'Transferred' THEN 'Archived' ELSE wrk.pipeline_name END AS pipeline_name, " +
                                                     "                              wrk.data_type, " +
                                                     "                              wrk.comments, " +
                                                     "                              wrk.details, " +
                                                     "                              wrk.justification, " +
                                                     "                              wrk.launch_time, " +
                                                     "                              wrk.status, " +
                                                     "                              wrk.step_description, " +
                                                     "                              wrk.percentagecomplete, " +
                                                     "                              insert_date, " +
                                                     "                              last_modified, " +
                                                     "                              isd.scanner, " +
                                                     "                              COALESCE(wrk.create_user, u.login)                                                     AS create_user, " +
                                                     "                              COALESCE(expt.label, subj.label, wrk.id)                                               AS label, " +
                                                     "                              COALESCE(es.project, ss.project)::varchar                                              AS shared_project, " +
                                                     "                              COALESCE(xes.code, xes.singular, xes.element_name)                                     AS type_desc " +
                                                     "                       FROM wrk_workflowData wrk " +
                                                     "                                LEFT JOIN wrk_workflowdata_meta_data meta ON wrk.workflowData_info = meta.meta_data_id " +
                                                     "                                LEFT JOIN xdat_user u ON meta.insert_user_xdat_user_id = u.xdat_user_id " +
                                                     "                                LEFT JOIN xnat_experimentData expt ON wrk.id = expt.id " +
                                                     "                                LEFT JOIN xnat_imagesessiondata isd ON isd.id = expt.id " +
                                                     ...
                                                     "                                LEFT JOIN xnat_subjectData subj ON wrk.id = subj.id " +
                                                     "                                LEFT JOIN xnat_projectparticipant ss ON subj.id = ss.subject_id " +
                                                     "                       WHERE expt.id IS NOT NULL " +
                                                     "                         AND wrk.category != 'SIDE_ADMIN' " +
                                                     "                         AND wrk.launch_time > now() - make_interval(days := :" + PARAM_NUM_DAYS + ") " +
                                                     "                         AND wrk.status != 'Failed (Dismissed)' " +
                                                     "                         AND wrk.pipeline_name !~ '^(.*/.*|Deleted)$'), " +
                                                     "           permSub1 AS (SELECT * FROM data_type_fns_get_user_access_by_project(:" + PARAM_ID + ")), " +
                                                     "           permSubQ AS (SELECT DISTINCT data_type, project, shared " +
                                                     "                        FROM permSub1 p " +
                                                     "                        WHERE p.data_type NOT IN ('xnat:projectData', 'xnat:subjectData') " +
                                                     "                          AND p.can_read) " +
                                                     "      SELECT wrk_workflowdata_id, " +
                                                     "             label, " +
                                                     "             id, " +
                                                     "             COALESCE(shared_project, externalid) AS externalid, " +
                                                     "             pipeline_name, " +
                                                     "             wrkSubQ.data_type, " +
                                                     "             scanner                              AS comments, " +
                                                     "             details, " +
                                                     "             type_desc                            AS justification, " +
                                                     "             launch_time, " +
                                                     "             status, " +
                                                     "             step_description, " +
                                                     "             percentagecomplete, " +
                                                     ...
                                                     "             create_user " +
                                                     "      FROM permSubQ " +
                                                     "               LEFT JOIN wrkSubQ ON (permSubQ.data_type = wrkSubQ.data_type AND NOT permSubQ.shared AND permSubQ.project = wrkSubQ.externalId) OR " +
                                                     "                                    (permSubQ.data_type = wrkSubQ.data_type AND permSubQ.shared AND permSubQ.project = wrkSubQ.shared_project AND wrkSubQ.shared_project != wrkSubQ.externalId) " +
                                                     "      WHERE wrk_workflowdata_id IS NOT NULL) AS q " +
                                                     "ORDER BY last_modified DESC " +
                                                     "LIMIT :" + PARAM_ROW_LIMIT + " OFFSET :" + PARAM_ROW_OFFSET;
    ...
    public List<Workflow> getWorkflows(String id, String dataType, UserI user,
        ...
        if (StringUtils.isNotBlank(id)) {
            namedParams.addValue(PARAM_ID, id);
        }

        String query;
        switch (dataType) {
            case "recent":
                namedParams.addValue(PARAM_NUM_DAYS, request.getDays() > 0 ? request.getDays() : 30)
                           .addValue(PARAM_ROW_LIMIT, request.getPageSize())
                           .addValue(PARAM_ROW_OFFSET, request.getOffset());
                query = QUERY_RECENT_EXPTS;
            ...
            case XnatProjectdata.SCHEMA_ELEMENT_NAME:
                namedParams.addValue(PARAM_ARC_ID,
    ...
    private String makeQueryPerUserPermissions(UserI user,
        ...
        String limitSuffix = "";
        if (!request.getSortable()) {
            namedParams.addValue(PARAM_ROW_LIMIT, request.getPageSize())
                       .addValue(PARAM_ROW_OFFSET, request.getOffset());
